...
class CashRegisterUtil extends Util
{
     ...
     * @return array
     */
    public function getRegisterTransactionDetails($user_id, $open_time, $close_time, $is_types_of_service_enabled = false, $business_id = null)
    {
        $product_details_by_brand = Transaction::where('transactions.business_id', $business_id)
        ...
        $product_details = Transaction::where('transactions.business_id', $business_id)
                ->whereBetween('transactions.transaction_date', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
                ->where('transactions.status', 'final')
                ->join('transaction_sell_lines AS TSL', 'transactions.id', '=', 'TSL.transaction_id')
        ...
        if ($is_types_of_service_enabled) {
            $types_of_service_details = Transaction::where('transactions.business_id', $business_id)
                ->whereBetween('transactions.transaction_date', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
                ->where('transactions.status', 'final')
        ...
        $transaction_details = Transaction::where('transactions.business_id', $business_id)
                ->whereBetween('transactions.transaction_date', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
                ->where('transactions.status', 'final')
                ->select(
