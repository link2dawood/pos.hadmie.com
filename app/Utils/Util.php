...
class Util
{
    ...
    public function payment_types($location = null, $show_advance = false, $business_id = null)
    {
        if (! empty($location)) {
            ...
            if ($location && !empty($location->business_id)) {
                $business = Business::find($location->business_id);
                if ($business && !empty($business->custom_labels)) {
        ...
        // Ensure custom_labels is always an array
    ...
    public function getCustomLabels($business, $type = null)
    {
        ...
        if (!$business) {
            \Log::warning('getCustomLabels called with non-existent business');
            return !empty($type) ? [] : [];
        }
        
        //Get custom label from business settings
        $custom_labels = $business->custom_labels ?? null;
