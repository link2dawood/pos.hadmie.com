...
class BusinessUtil extends Util
{
    ...
    public function newBusinessDefaultResources($business_id, $user_id)
    {
        ...
        // Grant account.access permission to Admin role by default
        $account_permission = Permission::firstOrCreate(['name' => 'account.access']);
    ...
    public function getCurrentFinancialYear($business_id)
    {
        if (empty($business_id)) {
            \Log::warning('getCurrentFinancialYear called with empty business_id');
            // Return default financial year (calendar year) if business_id is empty
        ...
        if (!$business) {
            \Log::error('Business not found in getCurrentFinancialYear. Business ID: ' . $business_id);
            // Return default financial year (calendar year) if business doesn't exist
            $start_date = date('Y-01-01');
            $end_date = date('Y-12-31');
            return [
                'start' => $start_date,
        ...
        // Use default financial year start month if not set (default to January)
