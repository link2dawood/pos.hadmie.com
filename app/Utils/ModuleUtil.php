...
class ModuleUtil extends Util
{
    ...
    /**
     * Get list of all available modules (from modules_statuses.json)
     * 
     * @return array
     */
    public function getAvailableModulesList()
    {
        $modules_status_file = base_path('modules_statuses.json');
        
        if (!file_exists($modules_status_file)) {
            return [];
        }
        
        $modules_status = json_decode(file_get_contents($modules_status_file), true);
        
        if (!is_array($modules_status)) {
            return [];
        }
        
        // Return only modules that are installed (value is true)
        $available_modules = [];
        foreach ($modules_status as $module_name => $is_installed) {
            if ($is_installed && $module_name !== 'Superadmin') { // Exclude Superadmin as it's always available
    ...
    /**
     * Check if a business has access to a specific module based on subscription
     * 
     * @param int $business_id
     * @param string $module_name
     * @return bool
     */
    public function hasModuleAccess($business_id, $module_name)
    {
        ...
        if (!$this->isSuperadminInstalled()) {
            // If superadmin module is not installed, allow all modules
            return true;
        }
        ...
        // Get package details from subscription
        $package_details = $subscription->package_details;
        
        // Check if modules array exists in package_details
        if (isset($package_details['modules']) && is_array($package_details['modules'])) {
            // If modules array is empty, allow all modules (backward compatibility)
            if (empty($package_details['modules'])) {
                return true;
            }
            // Check if module is in the allowed list
            return in_array($module_name, $package_details['modules']);
        }

        // If modules field doesn't exist, check package directly
        $package = $subscription->package;
        if ($package && !empty($package->modules) && is_array($package->modules)) {
            return in_array($module_name, $package->modules);
        }

        // Backward compatibility: if no modules restriction, allow access
        return true;
    }
