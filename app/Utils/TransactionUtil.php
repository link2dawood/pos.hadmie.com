...
class TransactionUtil extends Util
{
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        //Served by info (who actually served the customer)
        $output['sold_by'] = '';
        if (! empty($transaction->sales_person->user_full_name)) {
            $output['sold_by'] = $transaction->sales_person->user_full_name;
        } elseif (! empty($transaction->created_by)) {
            // Fallback to the user who created the transaction
            $created_by_user = \App\User::find($transaction->created_by);
            if ($created_by_user) {
                $output['sold_by'] = $created_by_user->user_full_name ?? $created_by_user->first_name . ' ' . $created_by_user->last_name;
            }
        }
        ...
        //Always provide amount due fields to avoid undefined properties in views
        $output['total_due_label'] = $il->total_due_label ?? trans('sale.total_due');
        $output['total_due'] = $this->num_f($transaction->final_total, $show_currency, $business_details);
        $output['total_paid'] = 0;
        $output['total_paid_label'] = $il->paid_label ?? trans('sale.total_paid');
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            //Qr code related information - Always enabled with structured newline-separated format
            ...
            if ($zatca_qr) {
                ...
                // Build structured QR code data - newline separated for easy reading
                $qr_code_lines = [];
                
                // Business Information
                if (!empty($business_details->name)) {
                    $qr_code_lines[] = $business_details->name;
                }
                if (!empty($location_details->name)) {
                    $qr_code_lines[] = $location_details->name;
                }
                
                // Address - use output address if available, otherwise build from location
                if (!empty($output['address'])) {
                    $qr_code_lines[] = $output['address'];
                } else {
                    ...
                    if (!empty($address_parts)) {
                        $qr_code_lines[] = implode(', ', $address_parts);
                    }
                }
                
                // Invoice Information
                if (!empty($transaction->invoice_no)) {
                    $qr_code_lines[] = $transaction->invoice_no;
                }
                if (!empty($output['invoice_date'])) {
                    $qr_code_lines[] = $output['invoice_date'];
                }
                
                // Customer Information
                $customer_name = !empty($output['customer_name']) ? $output['customer_name'] : (!empty($customer->full_name) ? $customer->full_name : 'Walk-in Customer');
                $qr_code_lines[] = $customer_name;
                
                // Financial Information
                if (!empty($output['subtotal'])) {
                    $qr_code_lines[] = $output['subtotal'];
                }
                if (!empty($output['total'])) {
                    $qr_code_lines[] = $output['total'];
                }
                if (!empty($output['tax'])) {
                    $qr_code_lines[] = $output['tax'];
                }
                
                // Tax Number
                $tax_number = !empty($business_details->tax_number_1) ? $business_details->tax_number_1 : 'N/A';
                $qr_code_lines[] = $tax_number;
                
                // Invoice URL
                $invoice_url = $this->getInvoiceUrl($transaction->id, $business_details->id);
                if (!empty($invoice_url)) {
                    $qr_code_lines[] = $invoice_url;
                }
                
                // Join with newlines for well-formatted, modular structure
                $qr_code_text = implode("\n", $qr_code_lines);
            }

            // Always set QR code text (not conditional on status)
