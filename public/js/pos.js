        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (result.success == 1) {
                    ...
                    // Always enable printing by default - like Ctrl+P behavior
                    if (result.receipt && result.receipt.html_content) {
                        pos_print(result.receipt);
                    } else if (result.receipt) {
                        // Force enable printing even if not explicitly enabled
                        result.receipt.is_enabled = true;
                        pos_print(result.receipt);
                    }
                } else {
                    toastr.error(result.msg);
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        if (result.success == 1) {
                            ...
                            // Always enable printing by default - like Ctrl+P behavior
                            if (result.receipt && result.receipt.html_content) {
                                console.log('Triggering print with html_content');
                                pos_print(result.receipt);
                            } else if (result.receipt) {
                                console.log('Triggering print with forced enabled');
                                // Force enable printing even if not explicitly enabled
                                result.receipt.is_enabled = true;
                                pos_print(result.receipt);
                            } else {
                                console.log('No receipt found in result');
                            }
                        } else {
                            toastr.error(result.msg);
                        }
...
function pos_print(receipt) {
    ...
    if (receipt && receipt.html_content) {
        console.log('Printing receipt with html_content');
        var title = document.title;
        if (typeof receipt.print_title != 'undefined') {
            document.title = receipt.print_title;
        }

        // Always print content - ignore printer type settings
        $('#receipt_section').html(receipt.html_content);
        __currency_convert_recursively($('#receipt_section'));
        console.log('Calling __print_receipt');
        ...
        console.log('No receipt or html_content found');
    }
    
    // Legacy code for printer type (commented out - always use browser printing)
    /*
