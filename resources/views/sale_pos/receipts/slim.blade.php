<div style="width: 100%; color: #000000 !important; font-family: Arial, sans-serif;padding: 10px;">
    ...
    <div style="margin-bottom: 30px; text-align: center;">
        ...
        <div style="margin-bottom: 10px;">
                ...
                <img style="max-height: 80px; width: auto;" src="{{ asset($receipt_details->logo) }}" alt="Company Logo">
            @else
        ...
        <div style="font-size: 18px; color: #000; margin-bottom: 20px;">
            ...
            @if(!empty($receipt_details->mobile))
                <br>Telephone: {{ $receipt_details->mobile }}
                @endif
            @if(!empty($receipt_details->email))
                <br>Email: {{ $receipt_details->email }}
                @endif
    ...
    <div style="margin-bottom: 25px;">
        <table style="width: 100%; border-collapse: collapse;">
            <tr>
                <td style="padding: 8px 0; font-weight: bold; width: 33%; font-size: 20px; text-align: left;">INVOICE NUMBER</td>
                <td style="padding: 8px 0; font-weight: bold; width: 33%; font-size: 20px; text-align: center;">TAX No.</td>
                <td style="padding: 8px 0; font-weight: bold; width: 33%; font-size: 20px; text-align: right;">DATE OF ISSUE</td>
            </tr>
            <tr>
                <td style="padding: 8px 0; font-size: 20px; text-align: left;">{{ $receipt_details->invoice_no }}</td>
                <td style="padding: 8px 0; font-size: 20px; text-align: center;">{{ $receipt_details->tax_number ?? 'N/A' }}</td>
                <td style="padding: 8px 0; font-size: 20px; text-align: right;">{{ $receipt_details->invoice_date }}</td>
            </tr>
        </table>
            </div>
    ...
    <div style="margin-bottom: 25px;">
        <table style="width: 100%; border-collapse: collapse; border-top: 2px solid #000;">
                ...
                <tr style="background-color: #f8f9fa;">
                    ...
                    <th style="padding: 12px 8px; text-align: right; font-weight: bold; border-bottom: 1px solid #ddd; font-size: 20px;">Price</th>
    ...
    <div style="margin-bottom: 25px;">
        <table style="width: 100%; border-collapse: collapse; background-color: #f8f9fa; border: 2px solid #000;">
            <tr style="background-color: #e9ecef;">
                ...
                <td style="padding: 12px; text-align: right; font-weight: bold; font-size: 20px;">{{ $receipt_details->subtotal ?? $receipt_details->subtotal_exc_tax }}</td>
                        </tr>
                    @if(!empty($receipt_details->taxes))
                        @foreach($receipt_details->taxes as $k => $v)
                <tr>
                    <td style="padding: 12px; font-size: 18px;">{{ $k }}</td>
                    <td style="padding: 12px; text-align: right; font-size: 18px;">{{ $v }}</td>
                            </tr>
                        @endforeach
            @elseif(!empty($receipt_details->tax))
            <tr>
                <td style="padding: 12px;">{{ $receipt_details->tax_label ?? 'Tax' }}</td>
                <td style="padding: 12px; text-align: right;">{{ $receipt_details->tax }}</td>
            </tr>
                    @endif
                    @if(!empty($receipt_details->discount))
            <tr>
                <td style="padding: 12px;">{{ $receipt_details->discount_label ?? 'Discount' }}</td>
                <td style="padding: 12px; text-align: right;">-{{ $receipt_details->discount }}</td>
                        </tr>
                    @endif
            <tr style="background-color: #000; color: #fff;">
                <td style="padding: 12px; font-weight: bold;">Total</td>
                <td style="padding: 12px; text-align: right; font-weight: bold; font-size: 20px;">{{ $receipt_details->total ?? $receipt_details->final_total }}</td>
                        </tr>
            <tr style="background-color: #e9ecef;">
    ...
    <div style="margin-bottom: 25px;">
        ...
        <div style="font-style: italic; color: #666;">
            {{ $receipt_details->total_in_words ?? 'N/A' }}
        </div>
    </div>
        ...
        @if($receipt_details->show_barcode || $receipt_details->show_qr_code)
            ...
            @if($receipt_details->show_qr_code && !empty($receipt_details->qr_code_text))
                <div style="margin: 0 auto; text-align: center;">
                    <img src="data:image/png;base64,{{DNS2D::getBarcodePNG($receipt_details->qr_code_text, 'QRCODE,M', 5, 5, [39, 48, 54])}}" style="width: 120px; height: 120px; display: block; margin: 0 auto;">
                </div>
