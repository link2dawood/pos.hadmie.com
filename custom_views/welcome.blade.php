                    ...
                    <form method="POST" action="{{ route('login') }}" id="login-form">
                        ...
                        <div class="form-group has-feedback {{ $errors->has('username') ? ' has-error' : '' }}">
                            <label class="tw-dw-form-control">
                                ...
                                <input
                                        class="tw-border tw-border-[#D1D5DA] tw-outline-none tw-h-12 tw-bg-transparent tw-rounded-lg tw-px-3 tw-font-medium tw-text-black placeholder:tw-text-gray-500 placeholder:tw-font-sm"
                        ...
                        @endif
                        <button type="submit"
                                class="tw-bg-gradient-to-r tw-bg-primary-800 tw-h-12 tw-rounded-xl tw-text-sm md:tw-text-base tw-text-white tw-font-semibold tw-w-full tw-max-w-full mt-2 hover:tw-bg-orange-700 focus:tw-outline-none focus:tw-ring-2 focus:tw-ring-blue-500 focus:tw-ring-offset-2 active:tw-from-indigo-700 active:tw-to-blue-700">
                            @lang('lang_v1.login')
                        </button>
                    </form>
                    ...
                    <div class="tw-flex tw-items-center tw-flex-col">
                        ...
                        @if (!($request->segment(1) == 'business' && $request->segment(2) == 'register'))
                            ...
                            @if (config('constants.allow_registration'))
                                <a href="{{ action([\Modules\Superadmin\Http\Controllers\PricingController::class, 'index']) }}@if (!empty(request()->lang)) {{ '?lang=' . request()->lang }} @endif"
