				...
				<div class="col-sm-12">
					<div class="form-group">
						<label style="font-weight: bold; margin-bottom: 10px; display: block;">
							@lang('superadmin::lang.allowed_modules') (@lang('superadmin::lang.leave_empty_for_all'))
						</label>
						<div class="row">
							@php
								$selected_modules = !empty($packages->modules) ? $packages->modules : [];
							@endphp
							@foreach($available_modules as $module_name => $module_display)
							<div class="col-sm-3">
								<div class="checkbox">
									<label>
										{!! Form::checkbox("modules[]", $module_name, in_array($module_name, $selected_modules), ['class' => 'input-icheck']); !!}
										{{ $module_display }}
									</label>
								</div>
							</div>
						...
						<span class="help-block">
							@lang('superadmin::lang.module_selection_help')
						</span>
					</div>
				</div>
				<div class="clearfix"></div>
