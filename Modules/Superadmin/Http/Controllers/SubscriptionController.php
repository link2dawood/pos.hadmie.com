...
class SubscriptionController extends BaseController
{
    ...
    public function confirm($package_id, Request $request)
    {
        ...
        try {
            ...
            $merchant_ref = $request->merchant_reference ?? $request->input('pesapal_merchant_reference');
            if (isset($this->_payment_gateways()['pesapal']) && $merchant_ref && (strpos($merchant_ref, 'PESAPAL') !== false)) {
                $transaction_id = $request->input('pesapal_transaction_tracking_id') ?? $package_id;
                return $this->confirm_pesapal($transaction_id, $request);
            }
            ...
            // CRITICAL: Verify business exists before proceeding
            if (empty($business_id)) {
                \Log::error('Business ID is empty in session. Cannot create subscription.');
                throw new \Exception('Business ID not found in session. Please login again.');
            }
            
            $business_exists = \DB::table('business')->where('id', $business_id)->exists();
            if (!$business_exists) {
                \Log::emergency('CRITICAL: Business does not exist in database! Business ID: ' . $business_id);
                \Log::emergency('This may be from a failed registration attempt. Cannot create subscription.');
                throw new \Exception('Business ID ' . $business_id . ' does not exist in database. Please contact support.');
            }
            
            \Log::info('Business verified before payment. Business ID: ' . $business_id);
    ...
    /**
     * Handle pesapal callback - verify transaction and update subscription
     *
     * @return Response
     */
    public function pesapalCallbackHandler(Request $request)
    {
        try {
            $merchant_reference = $request->input('pesapal_merchant_reference');
            $transaction_tracking_id = $request->input('pesapal_transaction_tracking_id');
            
            \Log::info('=== PESAPAL CALLBACK HANDLER ===');
            \Log::info('Merchant Reference: ' . $merchant_reference);
            \Log::info('Transaction Tracking ID: ' . $transaction_tracking_id);
            
            // Get package_id from session
            $pesapal_session = $request->session()->get('pesapal');
            
            if (!$pesapal_session || !isset($pesapal_session['package_id'])) {
                \Log::error('Pesapal session data missing');
                return redirect()
                    ->action([\Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'index'])
            ...
            $session_ref = $pesapal_session['ref'];
            
            // Verify merchant reference matches session
            if ($session_ref != $merchant_reference) {
                \Log::error('Merchant reference mismatch. Session: ' . $session_ref . ', Request: ' . $merchant_reference);
                return redirect()
                    ->action([\Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'index'])
                    ->with('status', ['success' => 0, 'msg' => __('messages.something_went_wrong')]);
            }
            
            // Get business and user info from session
            $business_id = $request->session()->get('user.business_id');
            $user_id = $request->session()->get('user.id');
            
            if (!$business_id || !$user_id) {
                \Log::error('Business ID or User ID missing from session');
                return redirect()
                    ->action([\Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'index'])
                    ->with('status', ['success' => 0, 'msg' => __('messages.something_went_wrong')]);
            }
            ...
            $package = Package::active()->find($package_id);
            if (!$package) {
                \Log::error('Package not found. Package ID: ' . $package_id);
                return redirect()
                    ->action([\Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'index'])
                    ->with('status', ['success' => 0, 'msg' => __('messages.something_went_wrong')]);
            }
            ...
            try {
                ...
                $is_live = config('pesapal.live', true);
                ...
                if (isset($token_result['token'])) {
                    ...
                    $status_url = $base_url . '/api/Transactions/GetTransactionStatus?orderTrackingId=' . $transaction_tracking_id;
                    ...
                    $transaction_status = json_decode($status_response, true);
                    curl_close($ch);
                    
                    \Log::info('Pesapal transaction status: ' . json_encode($transaction_status));
                    
                    // Check various possible status fields
                    $status = null;
                    if (isset($transaction_status['payment_status_description'])) {
                        $status = strtoupper($transaction_status['payment_status_description']);
                    } elseif (isset($transaction_status['status'])) {
                        $status = strtoupper($transaction_status['status']);
                    } elseif (isset($transaction_status['payment_status'])) {
                        $status = strtoupper($transaction_status['payment_status']);
                    }
                    ...
                    if ($status == 'COMPLETED' || $status == 'COMPLETE') {
                        ...
                        \Log::info('Payment status: ' . ($status ?? 'UNKNOWN'));
                    }
                } else {
                    \Log::error('Failed to get Pesapal access token');
                }
            } catch (\Exception $e) {
                \Log::error('Error getting Pesapal transaction status: ' . $e->getMessage());
                \Log::error('Stack trace: ' . $e->getTraceAsString());
                // If verification fails, proceed with creating subscription but set to waiting
            }
            
            // Check if subscription already exists
            $subscription = Subscription::where('payment_transaction_id', $transaction_tracking_id)
                ->where('business_id', $business_id)
                ->first();
            
            if ($subscription) {
                // Update existing subscription
                \Log::info('Updating existing subscription. Subscription ID: ' . $subscription->id);
                
                if ($payment_completed) {
                    // Payment completed - approve subscription
                    if ($subscription->status != 'approved') {
                        ...
                        $subscription->save();
                        \Log::info('Subscription approved. Subscription ID: ' . $subscription->id);
                    }
                } else {
                    // Payment pending or failed - keep as waiting
                    if ($subscription->status != 'waiting') {
                        $subscription->status = 'waiting';
                        $subscription->save();
                        \Log::info('Subscription set to waiting. Subscription ID: ' . $subscription->id);
                    }
                }
            } else {
                // Create new subscription
                \Log::info('Creating new subscription');
                $this->_add_subscription($business_id, $package, 'pesapal', $transaction_tracking_id, $user_id);
                
                // If payment is completed, update subscription status immediately
                if ($payment_completed) {
                    $subscription = Subscription::where('payment_transaction_id', $transaction_tracking_id)
                        ->where('business_id', $business_id)
                        ->first();
                    ...
                    if ($subscription) {
                        ...
                        $subscription->trial_end_date = $dates['trial'];
                        $subscription->save();
                        \Log::info('New subscription created and approved. Subscription ID: ' . $subscription->id);
                    }
                }
            }
            
            // Clear pesapal session
            $request->session()->forget('pesapal');
            ...
            \Log::error('Error in pesapalCallbackHandler: ' . $e->getMessage());
            \Log::error('Stack trace: ' . $e->getTraceAsString());
    ...
    {
        $merchant_reference = $request->merchant_reference ?? $request->input('pesapal_merchant_reference');
        $pesapal_session = $request->session()->pull('pesapal');

        if ($pesapal_session && $pesapal_session['ref'] == $merchant_reference) {
            $package_id = $pesapal_session['package_id'];
