...
class PackagesController extends Controller
{
    ...
    public function create()
    {
        ...
        $available_modules = $this->moduleUtil->getAvailableModulesList();

        return view('superadmin::packages.create')
            ->with(compact('intervals', 'currency', 'permissions', 'available_modules'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Handle modules - convert to array if provided
            if (isset($input['modules']) && is_array($input['modules'])) {
                $input['modules'] = array_filter($input['modules']); // Remove empty values
            } else {
                $input['modules'] = null;
            }
    ...
    public function edit($id)
    {
        ...
        return view('superadmin::packages.edit')
               ->with(compact('packages', 'intervals', 'permissions', 'available_modules'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $packages_details = $request->only(['name', 'id', 'description', 'location_count', 'user_count', 'product_count', 'invoice_count', 'interval', 'interval_count', 'trial_days', 'price', 'sort_order', 'is_active', 'custom_permissions', 'is_private', 'is_one_time', 'enable_custom_link', 'custom_link', 'custom_link_text', 'modules']);
            ...
            // Handle modules - convert to array if provided
            if (isset($packages_details['modules']) && is_array($packages_details['modules'])) {
                $packages_details['modules'] = array_filter($packages_details['modules']); // Remove empty values
            } else {
                $packages_details['modules'] = null;
            }
